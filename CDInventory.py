...
while True:
    ...
    if strChoice == 'l':
        ...
        for row in lstTbl:
            print(*row.values(), sep = ' | ')   #Updated to only print each value, asterisk unpacks each item from list of values 
    elif strChoice == 'd':
        ...
        for row in lstTbl:
            strRow = ''
            for item in row.values():   #Updated to only loop through items in values
                strRow += str(item) + ' | '   #str function not really needed since line 35 creates ID item as string
            strRow = strRow[:-1] + '\n'
